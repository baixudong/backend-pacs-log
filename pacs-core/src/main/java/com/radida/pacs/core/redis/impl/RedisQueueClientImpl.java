package com.radida.pacs.core.redis.impl;
...
import com.radida.pacs.core.redis.RedisQueueClient;
...
public class RedisQueueClientImpl implements RedisQueueClient {

	protected Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	protected JedisPool queueReadRedisPool;
	@Autowired
	...
	public Boolean addList(String key, List<Serializable> list) {
		if (key != null && list != null && list.size() > 0) {
			...
			boolean tag = false;
			try {
				writeRedisClient = (Jedis) queueWriteRedisPool.getResource();
			...
			if (writeRedisClient != null) {
				...
				try {
					pl = null;
					queueWriteRedisPool.returnResource(writeRedisClient);
	...
	public long getListLength(String key) {
		...
		if (key != null) {
			...
			try {
				readRedisClient = (Jedis) queueReadRedisPool.getResource();
			} catch (JedisConnectionException e) {
				logger.error("redis connection pool havn't idle contention !", e);
			}
			if (readRedisClient != null) {
				...
				try {
					queueReadRedisPool.returnResource(readRedisClient);
				} catch (RuntimeException e) {
					logger.error("redis connection pool couldn't return  connection !", e);
				}
			} else {
	...
	public List<Serializable> getList(String key) {
		if (key != null) {
			...
			if (readRedisClient != null) {
				...
				if (size != null && size > 0) {
					List<byte[]> results = readRedisClient.lrange(key.getBytes(), 0, size.intValue());
		...
		if (key != null) {
			...
			if (readRedisClient != null) {
				...
				if (results != null && results.size() > 0) {
					...
					logger.error("key={},start={},end={}  list not existing !", new Object[] { key, start, end });
				}
	...
	public Set<Serializable> getSet(String key) {
		if (key != null) {
			...
			if (readRedisClient != null) {
				Set<byte[]> resultSet = readRedisClient.smembers(key.getBytes());
	...
	public Boolean setListItems(String key, Integer[] indexes, Serializable[] values) {
		if (key != null && values != null && values.length > 0 && indexes != null && indexes.length > 0
	...
	public boolean set(String key, Serializable value) {
		...
		if (key != null && !"".equals(key)) {
			...
			if (writeRedisClient != null) {
				try {
					writeRedisClient.mset(key.getBytes(), getBytesFromObject(value));
					flag = true;
				} catch (RuntimeException re) {
	...
	public JedisPool getqueueReadRedisPool() {
		return queueReadRedisPool;
	}

	public void setqueueReadRedisPool(JedisPool queueReadRedisPool) {
		this.queueReadRedisPool = queueReadRedisPool;
	}

	public JedisPool getqueueWriteRedisPool() {
		return queueWriteRedisPool;
	}

	public void setqueueWriteRedisPool(JedisPool queueWriteRedisPool) {
		this.queueWriteRedisPool = queueWriteRedisPool;
	}

	@Override
	public boolean addSort(String key,double score, String values) {
		if (key != null ) {
			logger.debug("key:{} add  begin !", key);
			Jedis writeRedisClient = null;
			boolean tag = false;
			try {
				writeRedisClient = (Jedis) queueWriteRedisPool.getResource();
			...
			if (writeRedisClient != null) {
				try {
					writeRedisClient.zadd(key, score, values);
					tag = true;
				} catch (RuntimeException re) {
					logger.error("add begin error ! key {}", key);
					logger.error("add begin error !", re);
				}
				try {
					queueWriteRedisPool.returnResource(writeRedisClient);
				} catch (RuntimeException e) {
